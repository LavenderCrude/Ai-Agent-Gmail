...
async function gmailAuthenticate() {
  ...
  if (fs.existsSync(tokenPath)) {
    ...
    if (switchAccount === 'yes') {
      ...
      console.log(
        'Existing token deleted. A new authentication window will open.'
      );
    } else {
      return {
        service: google.gmail({ version: 'v1', auth: oauth2Client }),
      ...
      if (!body) {
        for (const part of full.data.payload.parts) {
          if (part.mimeType === 'text/html' && part.body?.data) {
            const bodyHtml = Buffer.from(part.body.data, 'base64').toString(
              'utf-8'
            );
...
async function sendReply(
  ...
  try {
    const encodedMsg = Buffer.from(msg)
      .toString('base64')
      .replace(/\+/g, '-')
      .replace(/\//g, '_')
      .replace(/=+$/, '');
  ...
  try {
    ...
    while (true) {
      try {
        ...
        if (!msgs.length) {
          console.log(
            `No new unread emails. Sleeping for ${pollInterval / 1000}s...`
          );
        ...
        for (const msg of msgs) {
          ...
          const structured = await callAIForStructured(
            data.subject,
            data.from,
            data.body
          );
          ...
          if (replyTemplate.should_reply) {
            ...
            const bodyReply =
              replyTemplate.body ||
              'Thank you for your email. I have received it and will get back to you shortly.';
            const fullMeta = await service.users.messages.get({
              userId: 'me',
              id: msgId,
              format: 'metadata',
            });
          ...
          if (
            structured.action === 'archive' ||
            structured.category === 'not_important'
          ) {
